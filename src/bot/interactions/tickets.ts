  ...
  client.on('interactionCreate', async (interaction: Interaction) => {
    ...
    try {
      ...
      await interaction.editReply({
        content: lang === 'es' 
          ? `<:check:1469545630957854731> Ticket creado: ${ticketChannel.toString()}`
      ...
      while (!otherUser && attempts < maxAttempts) {
        ...
        try {
          ...
          if (response.mentions.users.size > 0) {
            ...
            // Try to get by ID
            const userId = response.content.trim();
      ...
      await ticketChannel.send(lang === 'es'
        ? `${otherUser.toString()} | Usuario añadido a la transacción actual <:check:1469545630957854731>`
